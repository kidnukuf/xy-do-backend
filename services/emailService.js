  ...
  try {
    if (useResend) {
      ...
      const { data, error } = await resend.emails.send({
        from: 'XY-DO Athletic Programs <noreply@xy-do.com>',
        to: [email],
      ...
      console.log(`Verification email sent to ${email} via Resend:`, data.id);
      ...
      console.log(`Verification email sent to ${email} via nodemailer:`, info.messageId);
...
exports.sendWelcomeEmail = async (email, name, role) => {
  ...
  try {
    if (useResend) {
      ...
      console.log(`Welcome email sent to ${email} via nodemailer:`, info.messageId);
      return { success: true, messageId: info.messageId };
    }
  } catch (error) {
...
exports.sendPasswordResetEmail = async (email, name, resetToken) => {
  ...
  const htmlContent = `
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          <h2>Hi ${name},</h2>
          <p>We received a request to reset your password for your XY-DO Athletic Programs account.</p>
  ...
  const textContent = `Hi ${name},\n\nWe received a request to reset your password for your XY-DO Athletic Programs account.\n\nClick the link below to reset your password:\n\n${resetUrl}\n\nThis link will expire in 1 hour.\n\nIf you didn't request a password reset, please ignore this email.\n\nBest regards,\nXY-DO Team`;
  ...
  try {
    if (useResend) {
      const { data, error } = await resend.emails.send({
        from: 'XY-DO Athletic Programs <noreply@xy-do.com>',
        to: [email],
        subject: 'Reset Your XY-DO Password',
        html: htmlContent,
        text: textContent
      });
      ...
      console.log(`Password reset email sent to ${email} via Resend:`, data.id);
      return { success: true, messageId: data.id };
    } else {
      const info = await nodemailerTransporter.sendMail({
        from: `"XY-DO Athletic Programs" <${process.env.EMAIL_USER}>`,
      ...
      console.log(`Password reset email sent to ${email} via nodemailer:`, info.messageId);
      return { success: true, messageId: info.messageId };
    }
  } catch (error) {
